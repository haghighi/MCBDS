...
//ACU281 Code Section Begin
    ...
    int sim_time;
    bool is_in_Promisc_mode;
    bool is_in_waiting_4forward;
    int* baitList;
    int* suspectList;    /* address of node that 'Index' node want us to monitor it */
    int* suspectList_Mal;    /* address of node that 'Index' node guest it is Malicious */
    int* suspectListPackets; /*number of Data packet that Node 'Index' send to suspect node,*/
    int* suspectList_dest;   /* The destination node that Source wants send packet to him */
    int inform_SeqNumber;
    int* malicious_list;   // initial value is -1, and value of each home shows Id of malicious node
    int malicious_node_index;
    int* malicious_list_condidate;  /* we  are not sure that, node is Malicious. So we save it's ID,
                                        and wait for specific time for receiving response to Test Packets
                                        Index=malicious node's ID
                                        Value=Destination which it's packets must be passed though Mal_node
                                         */
    double* malicious_list_cond_time;    // Time which we suspected to Node
    int* malicious_list_cond_recv_pkt;  // Number of response received for Test Packets
    int* inform_seq_table;
    int initialBait;       // used as a flag, to ensure initial bait phase executed once
...
#define TAP_BITMASK	(TAP_CACHE_SIZE - 1)
  ...
  //ACU281 Code Section Begin
  void getRouteForPacketPRIME(ID bait, bool retry);
  // It's is like a normal RREQ, except it has bait address as destination addr
  void sendRecheckMessage(kPrime* tList,sr_addr mal_node, bool retry);
  //MCBDS
  void sendRecheckMessage_New(kPrimeList_New* tList, nsaddr_t mal_id, int destination);

    int test_packet_num;	// number for our next route_request
  void acceptRecheckMessage(SRPacket &p);
  void acceptTestPacket(SRPacket &p);
  void sendFakeRRep(SRPacket &p);
  void sendInformPacket(int malicious_ID);
  void handleInformMessage(SRPacket &p);
  bool ignoreInformMessage(int src, int seq);
  void inform_table_insert(int src, int seq);
  void malicious_list_insert(int malID);
  bool isMalicious(int src);
  void initiateBaitPhase();
  void sendTestPackets(Path pkt,int pkt_num);
  void myTimer(Event*);
  void myTimer2();
  //void go4Monitoring(const Packet * packet);
  //void sendRecheckResponse(int reciever,bool isSafe, sr_addr* address, int addr_size);
  void sendRecheckResponse(int reciever, bool isSafe, sr_addr* address, int addr_size, sr_addr to_monitor, sr_addr suspect_ID, int destination);
  void go4Monitoring(SRPacket packet);
  void recalculate_is_in();
  //MCBDS

// MCBDS
  //kPrime* calculateSList(kPrimeList* kpl);
  //bool contain(kPrime kp,sr_addr item);
  //kPrime* calculateTrustedList(sr_addr* repList,int& repListLength,kPrime* sList);
  //ACU281 Code Section End
