...
{
    trace("___________");
  trace("pre_noticeRouteUsed(const Path& p, Path& stub, Time t, const ID& who_from) start \t\t[Node %s]",net_id.dump());
trace(p.dump());
trace(stub.dump());
 ...
 trace("___________");
  trace("pre_noticeRouteUsed(const Path& p, Path& stub, Time t, const ID& who_from) end \t\t[Node %s]",net_id.dump());
